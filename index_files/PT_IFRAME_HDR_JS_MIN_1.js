...
var thisGetContxtRoot = function(strURL) {
 ...
 root = strURL.substr(rootIdx + 3); while(root.indexOf("\/\/") != -1)
 root = root.replace('\/\/','\/'); var srvlet = String(root).match(/\/ps(c|p)\//); if (srvlet != null) {
...
var ptBaseURI = getptBaseURI();var ptIframeHdr = {
 ...
 ptIframeHdr.isSafariOniPad = browserInfoObj2.isiPad && browserInfoObj2.isSafari; ptIframeHdr.atfLink = ptUtil.id("pthdr2atf");   var fluidLikeClassicHdrWithAddTo = document.getElementById("pthdr2addtowidget");  if (ptIframeHdr.atfLink || fluidLikeClassicHdrWithAddTo) {
 ...
 ptIframeHdr.linksParent = ptUtil.id("pthdr2links");  if (!ptIframeHdr.linksParent) ptIframeHdr.linksParent = ptUtil.id("pthdr2syslinks");  ptIframeHdr.addEvents(); ptIframeHdr.rtlSetWidth();    var srchInput = ptUtil.id("pthdr2srchinput"); if (srchInput && typeof ptASObj !== "undefined") {
 ...
 if (typeof hdrLinks[i].onclick != "function")
 {
 ...
 var newHref = goButton.href + "?SEARCH_TEXT=" + encodeURI(document.searchhdr.SEARCH_TEXT_HDR.value);   if (!ptIframeHdr.isHomepage && !ptUtil.isClassMember(this,"pthns")) { 
 ...
 return btnActivate();  }); ptEvent.add(goButton,"click", btnActivate); }
 }
 },

 onClickHdrLink : function(e) {
 return ptIframeHdr.hdrLinkClicked (this); },

 hdrLinkClicked : function(linkObj) {

 
 if (typeof(pthNav) !== "undefined" && !ptIframeHdr.isHomepage) { pthNav.mru.save(linkObj); }
 ...
 if (!ptIframeHdr.isHomepage && !ptUtil.isClassMember(linkObj, "pthns")) { 
 ptIframe.saveWarning(linkObj.href); } else {
 window.open(linkObj.href,"_parent"); }
 return false;  }
 ...
 if (!ptIframeHdr.isHomepage && !ptUtil.isClassMember(linkObj,"pthns")) { 
 var linkId = linkObj.id; if ((typeof(pthNav) !== "undefined" && typeof(pthNav.abn) !== "undefined") && 
 (linkId === "pthdr2home" || linkId === "pthdr2logout")) {
 ptIframe.saveWarning(linkObj.href, function(tgt) {

 if (linkId === "pthdr2logout") {
 pthNav.abn.search.clearData(); } 
 open(tgt,"_parent"); }); } else if (typeof(ptIframe) !== "undefined") {
 ptIframe.saveWarning(linkObj.href); } else if (typeof(saveWarning) !== "undefined") {
 
 saveWarning("TargetContent",document.forms[0],"_top", linkObj.href)
 } else {
 open(linkObj.href, "_parent"); }
 } else { 
 open(linkObj.href, "_parent"); }
 } 
 return false; },
 ...
 ptIframeHdr.favAddEvents(); ptIframeHdr.isFavFirstTime = false;  var bcList = ptUtil.id("pthbcUlScroll"); if (bcList && bcList.querySelector("li.ptabnportalfldr > a.pthnavbcanchor")) {
 ptIframeHdr.isSmartNav = true; }
 }
 ...
 baseURI = String(baseURI).replace('\/'+rnode+'\/','\/'+lnode+'\/'); }
  ...
  var sVal = "&ptatfsid=", s = ptUtil.id("ptatfsid");  if (s) { sVal += s.value; }
 ...
 + "&smartnav=" + smartNav
 + sVal; var loader = new net2.ContentLoader(ajaxURL,null,null,"GET",
 function () {
 
 var favSaveMsg = this.req.responseText; if (favSaveMsg !== "") {
 
 ptIframeHdr.favList.push(ptIframeHdr.favDescr.value);  ptIframeHdr.favPopup.message("",favSaveMsg);   if (typeof ptNav2 !== "undefined") {
 ptNav2.forceNavFavUpdate(); }

 if (typeof pthNav !== "undefined") { 
 pthNav.forceNavFavUpdate();  }
 ...
 if(!isWorkCenter())
 {
 ptIframeHdr.navLabel = decodeURI(favLabel); }
 ptIframeHdr.navObjName = favObjName; ptIframeHdr.isNavColl = isNavColl; },
...
var ptPopup = function (el,options) {
 ...
 popupWidth = popup.getIEPopupWidth(); c.style.width = popupWidth + "px";    } else if (!document.documentMode || document.documentMode !== 8) {
 ...
 setButtons : function (btnType) {

 if (typeof btnType === "undefined") { btnType = "OK_CANCEL"; }

 if (btnType === "YES_NO") {
  popup.msgBtn1.value = popup.msgBtn1.title = "Yes";  popup.msgBtn2.value = popup.msgBtn2.title = "No"; } else if (btnType === "OK_CANCEL") {
  popup.msgBtn1.value = popup.msgBtn1.title = "OK";  popup.msgBtn2.value = popup.msgBtn2.title = "Cancel"; } else { 
  popup.msgBtn1.value = popup.msgBtn1.title = "OK";  popup.msgBtn2.value = popup.msgBtn2.title = "Cancel"; }
 },
 ...
 if (popup.msg) {
 popup.setButtons(); this.btn1Handler = btn1Handler;  popup.msgTitle.replaceChild(document.createTextNode(title),
 ...
 popup.setButtons();  popup.msgTitle.replaceChild(document.createTextNode(title),
 ...
 this.btn1Handler = btn1Handler; this.btn2Handler = btn2Handler; popup.setButtons(btnType); if (!btnFocus) {
