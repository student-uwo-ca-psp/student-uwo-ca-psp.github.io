...
ptSidePanel.prototype = {
 ...
 if (typeof ptIframe == "undefined") return; ptUtil.addClass(this.objPageletAreaContainer, RCPage.CSS_RC_HIDDEN);  var WCPanelSize = ((typeof ptalPage != "undefined") && ptalPage && (ptalPage.objPageletArea) && (ptalPage.objPageletArea.style.right != "")) ? ptalPage.objPageletAreaContainer.clientWidth : 0; if (WCPanelSize > 0) {
 ...
 var initpage = true; if (!RCPage.initialized) 
 {
 initpage = false; RCPage.init();  }
 ptrc.bSavedRCExpanded = true; ptrc.bRCExpanded = true; ptrc.bRCHided = false;  if (!ptrc.bFirstSideFrameExpanded)
 ptrc.openMaxServices(id); if (!RCPage.visible)
 RCPage.showPageletArea(); RCPage.visible = true; if(initpage && ptrc.bInitiatedFromLink && !ptrc.bInitialServiceRequest)
 setTimeout("document.getElementById('vRCPgltAreaHide').focus();",1000); },
 ...
 {
 initpage = false; RCPage.init();  } 
 RCPage.visible = false; RCPage.hidePageletArea(pNoFading); ptUtil.removeClass(RCPage.objPageletAreaBodyAnchor , RCPage.CSS_RC_HIDDEN);  RCPage.objIconShowPageletArea .style.display = ""; ptrc.bRCExpanded = false; ptrc.bSavedRCExpanded = false; ptrc.bRCHided = true;  if(initpage && ptrc.bInitiatedFromLink && !ptrc.bInitialServiceRequest)
 setTimeout("document.getElementById('vRCPgltAreaShow').focus();",500); },
...
ptalPagelet.prototype = {
 ...
 msetparentfocus : function()
 {
 this.pgltAreaDocument.getElementById("ptalPgltAreaDropDown").focus(); },
 ...
 this.parentRCFrame = window.top.document.getElementById(this.typeRC + 'RelatedContent');  if (this.isSameComponent() && !this.bInitialServiceRequest)
 this.saveState(); else {
 this.selectedSeqNum = null; this.selectedServiceId = null; this.bRCHided = true; this.bRCExpanded = false; this.selectedURL = null; this.bInitialServiceRequest = true; if(this.bInitExpandedView)
 {
 this.bSavedRCHided = false;  this.bSavedRCExpanded = true; }
 else
 {
 this.bSavedRCHided = true;  this.bSavedRCExpanded = false; } 
 
 this.onAddChangeEvent(); }
...
mopen : function(event)
{
 ...
 this.mcancelclosetime();var el =document.getElementById("ptrcDropDowndiv_RelatedContent");var left=0;if(isClassicPlus())
var dropdown=document.getElementById("ptrcdropdownicon_RelatedContent");else
var dropdown=document.getElementById("ptsrc_dropdownicon_RelatedContent");while( dropdown != null && dropdown.id != "ptifrmrc") {
...
var rtladjust = 0;if ("ltr" == "rtl")
{
 rtladjust = ddmenuitem.offsetWidth - document.getElementById("ptrcdropdownicon_RelatedContent").offsetWidth; if (!browserInfoObj2.isIE) 
 rtladjust += 8; else
 rtladjust += 6;}
...
{
divpagecontrols.style.left= left + 2 + ele.clientWidth - ddmenuitem.offsetWidth + rtladjust + "px";divpagecontrols.style.top = top + 4 + "px";}
else
{
divpagecontrols.style.left= left + ele.clientWidth + 5 - ddmenuitem.offsetWidth + rtladjust + "px";divpagecontrols.style.top = top + 4 + "px";}
...
else if(key == 38) 
{
 ...
 try
 {
 var framesListForCP = document.getElementsByTagName('iframe'); for ( var i = 0; i < framesListForCP.length; i++ )
 {
 var frameElementForCP = framesListForCP[i]; var frameHtmlElementForCP = frameElementForCP.contentWindow.document.getElementsByTagName('html')[0]; if (frameHtmlElementForCP && frameHtmlElementForCP.className)
 {
 var classNameForCP = frameHtmlElementForCP.className; if( classNameForCP.indexOf('pt_classic_plus') !== -1 )
 {
 var rootElememntForCP = document.documentElement; ptUtil.addClass(rootElememntForCP, "pt_classic_plus"); }
 }
 break; }
 }
 catch(e){ }
 ...
 var classNameForCP = frameHtmlElementForCP.className; if( classNameForCP.indexOf('pt_classic_plus') !== -1 )
 {
 var rootElememntForCP = document.getElementById('SRelatedContent').contentWindow.document.getElementsByTagName('html')[0]; ptUtil.addClass(rootElememntForCP, "pt_classic_plus"); }
 }
 break; }
 }
 catch(e){ }
 ...
 if (doIt) {
 var activeEl = null; if(this.typeRC == "S" && this.bInitExpandedView && this.bInitialServiceRequest)
 {
 var targetframe= document.getElementById('ptifrmtgtframe'); if(typeof(targetframe) != 'undefined' && targetframe != null)
 activeEl = targetframe.contentWindow.document.activeElement; }
 ...
 if (!this.bInitExpandedView && (ptRC2.isFrame() || this.typeRC=="") )
 ...
 this.restoreState();     if (ptRC2.isIFrame() && (this.typeRC != "S")) {
 ...
 if(!browserInfoObj2.isIE) {
 if(typeof(activeEl) != 'undefined' && activeEl != null)
 activeEl.focus(); }
 this.bRefreshFromServer = false; }
 },
 ...
 if( rcFrameSrcPrefix.substr(rcFrameSrcPrefix.length - 14) == ".FieldFormula." ) {
 
 
 
 this.typeRC = "";  rcFrameSrcPrefix += "IScript_RelatedContent"; }
 ...
 this.bInitiatedFromLink = true; if(rc.ptPanelPagelet)
 rc.ptPanelPagelet.noPglts =0;  }, 
 ...
 window.top.ptrcMenu.closeRCMenu(); if ((nOpenMode == "0") || (nOpenMode == "2") || (nOpenMode == "3") || (nOpenMode == "4"))
 {
 onRCService(url,nOpenMode); return; }
 else {
 ...
 this.selectedURL = url; this.selectedSeqNum = seqNum; this.selectedServiceId = id; if (this.typeRC == "") {
 if (RCPage.visible)
 this.onHideRC(); }

 
 if (!this.bRCExpanded || this.bRCHided || (this.typeRC == "SF")) 
 this.onExpandRC("true", height);  if(this.typeRC == "" )
 {
 ...
 titleBar.style.width = (RCArea.offsetWidth+10) + "px";  }
 }
 else if(!this.bInitExpandedView || (this.bInitExpandedView && !this.bInitialServiceRequest && !this.bRefreshFromServer))
 {
 RCArea.onload = function()
 {
 var serviceTab = document.getElementById('ptrcServiceTab'); var items = serviceTab.getElementsByTagName('LI'); for(i=0; i<items.length; i++) {
 if(typeof(items[i].firstElementChild) != 'undefined' && items[i].firstElementChild != null &&
 items[i].firstElementChild.getAttribute("aria-selected") == "true")
 {
 items[i].firstElementChild.focus(); }
 }
 }
 }
 else
 ...
 {
 if(this.bInitiatedFromLink && RCPage.visible)
 setTimeout("document.getElementById('vRCPgltAreaHide').focus();",1000); }
 if (RCArea.src == url)
 this.bRefreshService = true;  if (this.bInitialServiceRequest || this.bRefreshService) {
 if (this.typeRC != ""){
 var pgltBody = RCArea.parentNode;  var htmlID = pgltBody.id.slice(10);  var bRCServRefresh = false; if ((pgltBody && /ptPgltMax/i.test(pgltBody.className)) && this.bRefreshService)
 bRCServRefresh = true;  setTimeout(function(){var bSave=(bDefault=="N")?1:0; rc.ptPanelPagelet.maximize(htmlID,bSave,bRCServRefresh);}, 0);  }
 ...
 fVal = fRef.value; } 
 if (URLParamsEscaped == 'Y' && fRef.innerHTML == "&nbsp;")
 ...
 {
 paramStr = paramStr + fp[i] + "=" + escape(fVal);  }
 else
 {
 ...
 return rcMenuStr; var rcMenuSpan = rcMenuOnTC.getElementsByTagName("span");  var rcServicePrefix = "<li><a role='tab' aria-selected='false' onkeydown='return doTabNav(event);' href=\""; var rcServiceSelectedPrefix = "<li id='selected'><a role='tab' aria-selected='true' onkeydown='return doTabNav(event);' href=\""; var rcServiceSuffix ="</span></a></li>"; var seqNum = null; var serviceId = null; var hrefURL = null; var serviceTitle = null; if (rcMenuSpan != null && rcMenuSpan.length >0) {
 this.rcMenu = new RCMenu(); rcMenuStr = "<div id='ptrcServiceTab'><ul role='presentation'>"; var tc = window.frames['TargetContent']; var isTangSS = false; for (var i=0; i < rcMenuSpan.length; i++)
 {
 if (rcMenuSpan[i].className == "ptrcMenuItem") {
 ...
 if (icoSettingsLnk && (icoSettingsLnk.title.indexOf(serviceName) == -1)) {
 icoSettingsLnk.title = serviceName + " " + icoSettingsLnk.title; }

 if (icoSettings) {
 icoSettings.alt = serviceName + " " + icoSettings.alt; }
 ...
 var rcMenuOnTC = this.getRCMenuOnTC(); if( typeof rcMenuOnTC!="undefined" && rcMenuOnTC){
 var rcMenuSpan = rcMenuOnTC.getElementsByTagName("span");  if (typeof rcMenuSpan!="undefined" && rcMenuSpan && rcMenuSpan.length >0) {
 var rcMenuStr = document.getElementById("ptrcServiceTab"); if(typeof rcMenuStr!="undefined"&& rcMenuStr){
 var rcMenuList= rcMenuStr.children[0]; if(typeof rcMenuList!="undefined" && rcMenuList){
 for(var i=0; i<rcMenuList.children.length; i++){
 var rcitemaTag = rcMenuList.children[i].getElementsByTagName('a')[0]; if(typeof rcitemaTag=="undefined" || rcitemaTag==null) continue; var rcitemaTaghref= rcitemaTag.href;  var params = rcitemaTaghref.split("?")[1].split("="); var paramlen = params.length; var serviceparam =params[paramlen-1].split(","); var slen = serviceparam[2].length; var serviceID = serviceparam[2].match(/'([^']+)'/)[1]; if(serviceID !=this.selectedServiceId)
 rcitemaTag.setAttribute("tabindex","-1");  }
 }
 }
 }
 }
 ...
 var height; var expCollImg = ptUtil.id("ptrccollapse_expand"); var titleStr = expCollImg.title; if (ptUtil.getCSSValue(rcBoby,"display") === "none") {
 rcBoby.style.display = "block"; titleStr = titleStr.replace("Expand","Collapse"); expCollImg.title = titleStr; this.bRCExpanded = true;    height = 0; ptUtil.swapClass(expCollImg,"ptrccollapse","ptrcexpand"); if (!this.bServiceLoadedInFrame)
 this.onRefresh();  } else {
 this.bRCExpanded = false; rcBoby.style.display = "none"; titleStr = titleStr.replace("Collapse","Expand"); expCollImg.title = titleStr; ptUtil.swapClass(expCollImg,"ptrcexpand","ptrccollapse");  var titleBar = ptUtil.id("ptrctitlebar"); var TCSpacer = ptUtil.id("ptrcTCSpacer"); height = titleBar.offsetHeight + TCSpacer.offsetHeight; }
...
onServiceByScroll:function (serviceName,scrollFieldName,bManual,bRefresh,nOpenMode,nInstID,sRecFldName,sPageFieldValues) {
 ...
 hrefURL = hrefURL.replace(sPnlFlds,sorigFieldNames); var sHREFPageFieldValues = this.getPageFieldValues(hrefURL); if ((typeof(sHREFPageFieldValues) != 'undefined') && (typeof(sPageFieldValues) != 'undefined'))
 {
 ...
 this.UpdateServURL(serviceName,hrefURL,sorigFieldNames,sNewFieldNames,nInstID,sPnlFlds);   if (bManual)
 ...
 UpdateServURL:function (servName,ServURL,sorigFieldNames,sNewFieldNames,nInstID,sPnlFlds) {
 var tc = window.frames['TargetContent']; if (tc == null) {
 return; }
 var rcMenuOnTC = this.getRCMenuOnTC(); if (rcMenuOnTC == null) {
 return; }
 ...
 {
 if (rcMenuSpan[i].className == "ptrcMenuItem") {
 hrefURL = this.getHrefURL(rcMenuSpan[i].innerHTML); var URLServiceName = this.getServiceNameFromURL(hrefURL); var servInstID = this.getServiceInstID(hrefURL); if (typeof(ptrc.szPageRCFFields) != 'undefined' && ptrc.szPageRCFFields != "")
 {
 var servLst = ptrc.szPageRCFFields.split("~"); for (var j=0; j < servLst.length; j++)
 {
 var nPos = servLst[j].indexOf(URLServiceName+"$"+servInstID); if (nPos!= -1)
 {
 var FldsLst = servLst[j].split("#"); var nPos1 = FldsLst[0].indexOf("^"); var sServdetails = FldsLst[0].substr(0,nPos1); var sPnlFldLst = FldsLst[0].substr(nPos1,FldsLst[0].length-nPos1); sorigFieldNames = this.getFieldNames(rcMenuSpan[i].innerHTML); rcMenuSpan[i].innerHTML = rcMenuSpan[i].innerHTML.replace(sorigFieldNames,FldsLst[1]); var sOrigPnlFlds = this.getPanleFieldNames(hrefURL); rcMenuSpan[i].innerHTML = rcMenuSpan[i].innerHTML.replace(sOrigPnlFlds,sPnlFldLst); }
 }
 if (this.rcMenu != null)
 {
 ...
 {
 if((URLServiceName == servName) && (servInstID == nInstID)) {
 sorigFieldNames = this.getFieldNames(rcMenuSpan[i].innerHTML); rcMenuSpan[i].innerHTML = rcMenuSpan[i].innerHTML.replace(sorigFieldNames,sNewFieldNames); var sOrigPnlFlds = this.getPanleFieldNames(hrefURL); rcMenuSpan[i].innerHTML = rcMenuSpan[i].innerHTML.replace(sOrigPnlFlds,sPnlFlds); if (this.rcMenu != null)
 {
 this.rcMenu.setValue(this.getServiceId(hrefURL),ServURL); this.onUpdateRCMenu(); }
 break; }
 }
...
setPageRCFFields:function (sPageRCFFields) {
ptrc.szPageRCFFields = sPageRCFFields;}
}
