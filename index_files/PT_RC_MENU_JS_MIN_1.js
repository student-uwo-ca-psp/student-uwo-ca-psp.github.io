...
toggle:function(evt,menuID) { 
 ...
 objCont.style.visibility ="visible"; var mnuLinks = objCont.getElementsByTagName("a"); if(mnuLinks.length > 0) {
 try {
 mnuLinks[0].focus(); curMnuLinks = 0; }catch(e){}
 }
 if (objCont) {
 ptEvent.add(objCont, "keypress", this.handleKeys); ptEvent.add(objCont, "keydown", this.handleKeys);  }
 if (document.all)
 ...
 evt.stopPropagation(); } 
 evt.cancelBubble = true; }catch(e){}

 }, 

handleKeys: function(pEvent) {
 var key = (window.event) ? window.event.keyCode: pEvent.keyCode; var tc = window.frames['TargetContent']; if (tc == null) {
 return; }
 if(key == 27) {
 ptrcMenu.toggle(key); var objRC = tc.document.getElementById("ptrcMenuALink");  if(objRC != null){
 objRC.focus(); return; }
 }
 var objCont = tc.document.getElementById("RCPAGEMENUCONTENT"); if(objCont != null){
 var mnuLinks = objCont.getElementsByTagName("a"); if(mnuLinks.length > 0) { 
 switch (key) {
 case 40:
 curMnuLinks++; if(curMnuLinks == mnuLinks.length)
 curMnuLinks = 0; break; case 38:
 curMnuLinks--; if(curMnuLinks == -1)
 curMnuLinks = mnuLinks.length-1; break; }
 try {
 if(key == 40 || key == 38){
 mnuLinks[curMnuLinks].focus(); if (pEvent.stopPropagation) {
 pEvent.stopPropagation(); } 
 pEvent.cancelBubble = true; if(pEvent.preventDefault) pEvent.preventDefault(); else pEvent.returnValue = false; }
 }catch(e){}
 }
 }
},
