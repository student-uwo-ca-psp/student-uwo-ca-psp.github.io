var listFormat = false;var ptBaseURI = getptBaseURI();var const_keyCode = { ARRLEFT : 37,ARRUP : 38,ARRRIGHT : 39,ARRDN : 40,ENTER : 13,
 ...
 ptEvent.add(pthNav.sortBtn,"click",pthNav.sort.handle);  pthNav.sort.initialize(); var accBTN = pthNav.sortBtn.getElementsByTagName("a"); if (accBTN)
 ...
 var el = seps[i];    if (selector == '#pthbcUlScroll' && el.innerHTML == '&nbsp;' && seps.length == 1) {
 el.innerHTML = "<span id='hiddenHPBC' style='visibility:hidden'>:</span>"; }
 else if (el !=null && el.innerHTML == '&nbsp;') {
 ...
 return;  var respHTML = this.req.responseText;     ptUtil.appendHTML(currNode,respHTML);    var flyout = ptUtil.id(pthNav.flyoutPrefix + fldrId); ptUtil.swapClass(flyout,"pthnavflyoutopen","pthnavbarshow"); if(flyout)
 pthNav.selectedBC = flyout.id;  var childEls = 0; childEls = pthNav.addEvents(flyout); if (childEls === 0 && flyout) {
 ...
 var UnPathPos = String(UniNavTgt).indexOf("uninavpath="); var UnParam = ""; var UniNavABNPCodeVal = ""; if (String(UniNavTgt).indexOf("cmd=uninav") == -1)
 UnParam = "&cmd=uninav";  if (isUnAbn == "true" && (typeof liCref.attributes['uninavabnpcode']) != "undefined" ){
 ...
 pthNav.closeNav(); var hdrFavEl = ptUtil.id("pthdr2atf") || ptUtil.id("addToFavoritesId"); if (hdrFavEl) {
 ...
 function () {
 if (pthNav.checkSignonResponse(this.req.getResponseHeader("RespondingWithSignonPage")))
 return;  var respHTML = this.req.responseText; if(!bcStartNode)return;  var bcParent = bcStartNode.parentNode;  if (!bcParent) {
 ...
 var bcUL = ptUtil.id("pthbcUlScroll"); if(bcUL)
 bcUpdater.setStoredData(bcUpdater.breadCrumbHTML, bcUL.innerHTML); }
 ...
 var newdiv = document.createElement('div'); newdiv.setAttribute('id',"dummydiv"); newdiv.setAttribute('style','none'); newdiv.innerHTML = UniNav.UniNavrespHTML; respHTML = newdiv.innerHTML; newdiv.innerHTML = ""; ptUtil.appendHTML(currNode,respHTML); pthNav.abn.displNavElem(currNode,"TRUE",UniNav.folderid,UniNav.parentId);  var childEls = 0; var flyout = ptUtil.id(pthNav.flyoutPrefix + currNode.id)
 childEls = pthNav.addEvents(flyout); if (childEls === 0 && flyout) {
 flyout.style.display = "none"; flyout.parentNode.firstChild.setAttribute("aria-haspopup", "false"); }
 if ((!pthNav.isNavOpen) || (!ptUtil.isClassMember(currNode, "pthnavfldropen"))) {
 ptUtil.removeClass(currNode,pthNav.loadStyle); pthNav.fldr.close(currNode); pthNav.lastHoveredId = ""; return; }
 ...
 displNavElem : function(node,uninav,folderID,parentID) {
 uninav = typeof uninav !== 'undefined' ? uninav : "FALSE"; folderID = typeof folderID !== 'undefined' ? folderID : "undefined"; parentID = typeof parentID !== 'undefined' ? parentID : "undefined"; if (!node) { return; }
 ...
 abnNavElemId = abnNavElemIdPreFix + node.id + abnNavElemIdPostFix;   if(uninav == "TRUE"){ 
 abnNavElemId = abnNavElemIdPreFix +folderID+ abnNavElemIdPostFix + "_"+parentID;  }
 abnLiNavElem = document.getElementById(abnNavElemId); if ( abnLiNavElem ) {
 ...
 s.win.popup.close();  pthNav.closeNav();  var srchType = document.getElementById("ptabnsrchtypeKW_" + s.currResults.objName); var srchMode = document.getElementById("ptabnsrchMode_" + s.currResults.objName); var srchCrit = document.getElementById("ptabnsrchCriteria_" + s.currResults.objName); var srchFilters = document.getElementById("ptabnsrchFilters_" + s.currResults.objName); var nSrchFldIndex = 0; var kw="&ICSesSrchTxt=";  var elmkeyw = document.getElementById("ptabnsrchKeywords"); if (srchType && (typeof(srchType) != "undefined") && (srchType.innerHTML == "Y") && (srchCrit.childNodes.length > 0) && elmkeyw) {
 nSrchFldIndex = 1; kw = "&ICSesSrchTxt=" + encodeURIComponent("" + elmkeyw.innerHTML); }
 ...
 var val = srchCrit.childNodes[i].innerHTML; var name1 = srchCrit.childNodes[i].id; var startLoc = name1.indexOf("ptabnsrchFld_"); if (startLoc == 0) {
 name1 = name1.substring(startLoc + 13); srchCritStr += "&" + name1 + "=" + encodeURIComponent("" + val); }
 }
 }
 ...
 name1 = name1.substring(startLoc1 + 21); srchFilStr += "&" + "FT_" + name1 + "=" + encodeURIComponent("" + val); }
 if (startLoc2 > -1) {
 name1 = name1.substring(startLoc2 + 16); srchFilStr += "&" + "FT_" + name1 + "=" + encodeURIComponent("" + val); }
 }
 }
