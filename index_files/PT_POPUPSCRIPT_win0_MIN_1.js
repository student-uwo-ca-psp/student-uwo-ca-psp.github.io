...
{
 this.tHandle = null; if (this.hoverpopupId == null || this.hoverpopupId=="" ) return; var hPop = document.getElementById(this.hoverpopupId);  var hp = document.getElementById(this.MOPopContainer);  var popupFrame = document.getElementById(this.MOPopFrame);  hp.style.display = "block"; popupFrame.innerHTML=hPop.innerHTML;  hPop.innerHTML = null;  popupFrame.style.dislay="block"; if (popupFrame)
...
ShowPopup:function() 
{
 this.tHandle = null; if (this.hoverpopupId == null || this.hoverpopupId=="" ) return; var hp = document.getElementById(this.hoverpopupId);  this.UpdatePopup(hp.innerHTML);  hp = document.getElementById(this.MOPopContainer);  var popupFrame = document.getElementById(this.MOPopFrame);    var offScreenElement = "<a class='ps-anchor' id='ICLastElement_win0' tabindex='0' onfocus='MOpopupObj_win0.RestoreTabSequence(event)'></a>"; popupFrame.innerHTML = popupFrame.innerHTML + offScreenElement; if (popupFrame)
 ...
 this.focusElem=null;   popupFrame.focus();},


RestoreTabSequence : function(event)
{
 var target = event.target; target.parentNode.focus();},
...
HidePopup:function() 
{
 ...
 var popupFrame = document.getElementById(this.MOPopFrame);  var hPop = document.getElementById(this.hoverpopupId);  hPop.innerHTML = popupFrame.innerHTML; popupFrame.innerHTML = null; var nLen = document.forms.length; for (var i=0; i<nLen; i++)
 {
...
UpdatePopup:function(sHTML) 
{
 ...
 if(isClassicPlus())
 {
 var updatedWidthForClassicPlus = this.MOcurrWidth * 0.22; this.MOcurrWidth = this.MOcurrWidth + updatedWidthForClassicPlus; }
 ...
 this.y = nHtop1 - this.MOcurrHeight-20;  }
 else
...
UpdatePopup:function(evt,sHTML,winname) 
{
 ...
 var viewportHeight = ptCommonObj2.getViewportHeight();  var viewportWidth = ptCommonObj2.getViewportWidth();  var scrolltop = (document.documentElement && document.documentElement.scrollTop) || 
 document.body.scrollTop;   if (this.bIE && (viewportWidth-30) > 60)
 viewportWidth = viewportWidth - 30;  else if (!this.bIE && (viewportWidth-50) > 60)
 ...
 this.HelpcurrWidth = viewportWidth; HelppopupObj_win0.actualStartPos = [evt.clientX,evt.clientY];  var nLeft=this.GetElementleft(evt, hItem);  if(nLeft < 3)
 nLeft = 3; if('ltr' == 'ltr') {
 nLeft = nLeft + 20; }
 else {
 if(this.bIE)
 ...
 this.x = nLeft;  var nTop=this.GetElementTop(evt, hItem); if((nTop + 40 + this.HelpcurrHeight) > (viewportHeight + scrolltop)) {
 ...
 if( (detectDoctype() && !bWSRP) || this.bIE10 )
