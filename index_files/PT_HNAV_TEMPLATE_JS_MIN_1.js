...
var ptIframe = {
 ...
 var rcMask=document.getElementById('ptalPageMask'); if(!rcMask){
 var rcMask = document.createElement("div"); rcMask.id="ptalPageMask"; rcMask.className ="ptalPageMask"; }

 var theBody = document.getElementsByTagName("body"); if (theBody && theBody.length)
 theBody[0].appendChild(rcMask);  if ((typeof ptalPage != "undefined") && (ptalPage)) {
 ptIframe.ptalPage = ptalPage; ptIframe.ptalPage.init(); }


 
 ptIframe.ptcxmceDiv = ptUtil.id("ptcxmcollexpdiv"); if (ptIframe.ptcxmceDiv) {

 ptIframe.ptcxmcAnc = ptUtil.id("ptcxmifrmsbcoll"); ptIframe.ptcxmeAnc = ptUtil.id("ptcxmifrmsbexp"); ptIframe.ptcxmpageletDiv = ptUtil.id("ptcxmpageletdiv"); ptIframe.tgtDiv = ptUtil.id("ptifrmtarget");  ptEvent.add(ptIframe.ptcxmcAnc,"click",function(){ptIframe.ptcxmToggleframe();});  ptEvent.add(ptIframe.ptcxmeAnc,"click",function(){ptIframe.ptcxmToggleframe();}); }

 ptIframe.tgtIframe = ptUtil.id("ptifrmtgtframe"); var contentParent = ptIframe.tgtIframe.parentNode.parentNode;   ptIframe.iframes = contentParent.getElementsByTagName("iframe"); var pNode, iframe, winWidth; var offsetWidth = 0, offsetWidthPercent = 0; for (var i = 0; i < ptIframe.iframes.length; i++) {
 ...
 iframe = ptIframe.iframes[i]; pNode = ptIframe.iframes[i].parentNode; if (pNode && pNode.nodeType === 1) {
 
 
 winWidth = ptIframe.winSize().width; offsetWidth = parseFloat(ptUtil.getCSSValue(pNode,"width")); ptIframe.offsetWidthPercent = offsetWidth / winWidth;  pNode.style.left = winWidth - offsetWidth + 2 + "px";  ptIframe.resizeWidth(iframe.name,2); ptIframe.resizeHeight(iframe.name,0);  ptIframe.resizeIframes.push(iframe); }
 }
 }
 ...
 ptIframe.resizeWidth("ptcxmiframe",0); ptIframe.resizeHeight("ptcxmiframe",0); ptIframe.resizeWidth("ptifrmtgtframe",200); ptIframe.resizeHeight("ptifrmtgtframe",0); }
 else {
 ptIframe.resizeWidth("ptcxmiframe",80); ptIframe.resizeHeight("ptcxmiframe",0); ptIframe.resizeWidth("ptifrmtgtframe",125); ptIframe.resizeHeight("ptifrmtgtframe",0); }
 }
 ...
 ptIframe.resizeWidth("ptifrmtgtframe",offsetWidth); ptIframe.resizeHeight("ptifrmtgtframe",0); }

 ptIframe.rcIframe = ptUtil.id("RelatedContent");  if (ptIframe.rcIframe) {
 ptIframe.rcDragHndl = ptUtil.id("ptifrmrchandle"); ptIframe.rcSep = ptUtil.id("ptifrmsep"); ptIframe.rcUpMask = ptUtil.id("ptifrmresizeupmask"); ptIframe.rcDownMask = ptUtil.id("ptifrmresizedownmask");  ptIframe.rc.dragResize.init(ptUtil.id("ptifrmrchandle"));  ptUtil.swapClass(ptIframe.rcIframe.parentNode,"ptifrmrcstd","ptifrmrcmax"); }

 
  ptEvent.add(window,"resize",function() {ptIframe.resizeAll();}); ptEvent.add(window,"load",function() {ptIframe.resizeAll();});  var popup = ptUtil.id("ptifrmpopup"); if (popup) {
 ...
 iframeHeight = ptIframe.tgtIframe.offsetHeight;  ptIframe.tgtIframe.style.height = ptIframe.tgtIframe.contentWindow.document.body.scrollHeight + "px";  });  ptEvent.add(window,"afterprint",function(){ptIframe.tgtIframe.style.height = iframeHeight + "px";});  }
  ...
  resizeAll: function(){
 var offsetWidth = 0; if (ptIframe.offsetWidthPercent !== 0) {
 var winWidth = ptIframe.winSize().width; offsetWidth = winWidth * ptIframe.offsetWidthPercent; var iframe = ptIframe.resizeIframes[0];  iframe.parentNode.style.left = winWidth - offsetWidth + 2 + "px";  ptIframe.resizeWidth(iframe.name,2); ptIframe.resizeHeight(iframe.name,0); }
 ...
 ptIframe.ptalPage.resize(); if ((typeof RCPage != "undefined") && RCPage && RCPage.initialized) {
 ptIframe.resizeWidth("ptifrmtgtframe",offsetWidth); RCPage.resize(); if (RCPage.visible) ptrc.resizeSideRCFrame(window.frames['SRelatedContent'], 20); }
 }
 else {
 ptIframe.resizeWidth("ptifrmtgtframe",offsetWidth); ptIframe.resizeHeight("ptifrmtgtframe",0); if ((typeof RCPage != "undefined") && RCPage && RCPage.initialized) {
 RCPage.resize(); if (RCPage.visible) ptrc.resizeSideRCFrame(window.frames['SRelatedContent'], 20); }
 }
 ...
 if (ptUtil.getCSSValue(ptIframe.ptcxmeAnc,"display") !== "none") {
  ...
  ptIframe.ptcxmeAnc.style.display = "none"; ptIframe.ptcxmcAnc.style.display = "inline"; ptIframe.ptcxmpageletDiv.style.display="inline"; ptUtil.swapClass(ptIframe.tgtDiv,"ptifrmtgtmax","ptcxmifrmtgtstd");  ptIframe.resizeWidth("ptifrmtgtframe",200); ptIframe.resizeHeight("ptcxmiframe",0); ptIframe.resizeHeight("ptifrmtgtframe",0); }
 else {
 
 ptIframe.ptcxmeAnc.style.display = "inline"; ptIframe.ptcxmcAnc.style.display = "none"; ptIframe.ptcxmpageletDiv.style.display="none"; ptUtil.swapClass(ptIframe.tgtDiv,"ptcxmifrmtgtstd","ptifrmtgtmax");  ptIframe.resizeWidth("ptifrmtgtframe",125); ptIframe.resizeHeight("ptifrmtgtframe",0); }
 ...
 var x = 0, y = 0, objX = 0;  var e = el; while(e) {
 ...
 var isWorkCenter = top.document.getElementById('ptalPgltAreaFrame');   var processCancel = function () {
 if (pthNav && pthNav.iframe) {
 try {
 var formName = pthNav.iframe.contentWindow.document.forms[0].name; if (!/empty/.test(formName) && (tgtName.toLowerCase() !== "_blank")) { 
 if ((typeof tgt == "string") && /javascript/i.test(tgt)) {
 ...
 return false; }; var processOk = function () {
 if (typeof(okFn) !== "undefined") {
 okFn.call(this); }
 return false; };  if (ptIframe.isDataChange()) {

 var warnMsg = "You have unsaved data on this page. Click OK to go back and save, or Cancel to continue.",
 btnType = "OK_CANCEL";  if (typeof isNewSaveWarn !== "undefined" && isNewSaveWarn) {
 warnMsg = "Do you want to save your changes? Click Yes to go back and save, or No to discard your changes."; btnType = "YES_NO"; }

 if (browserInfoObj2.isIE && ptIframeHdr.isAccessibility()) {
 var warnTitle = "Save Warning"; warnMsg = warnTitle + ". You have unsaved data on this page. Click OK to go back and save, or Cancel to continue."; if (confirm(warnMsg))
 processOk(); else
 processCancel(); } else 
 ptIframe.popup.prompt("Save Warning", warnMsg, btnType,
 processOk, 
 processCancel 
 );  return false; } else {
 processCancel(); }
 ...
 var pageletFrameObj = top.document.getElementById('ptalPgltAreaFrame'); if (!pageletFrameObj)
 return false; try {
 var tgtFrameForm = top.frames['TargetContent'].document.forms[0];  if (!tgtFrameForm)
 return false; if(!tgtFrameForm.ICChanged)
 return false; }
 catch(err) { return false; }
 
 if(tgtFrameForm.ICChanged.value === '-1')
 return false; var isDirty = ptIframe.isDataChange(); if (isDirty == "undefined") {
 isDirty = false; }
 ...
 var curAnchorObj = null; var bCallCancelFn = false; if(calledFrom === 1) { 
 ...
 return false; var anchorType = curAnchorObj.getAttribute("ptlinktgt"); if((anchorType) && (anchorType !== 'pt_replace')) 
 return false; } else if (calledFrom === 2){ 
 ...
 if(typeof curAnchorObj !== 'undefined' && curAnchorObj != null) {
 var anchorType = curAnchorObj.getAttribute("ptlinktgt");  if((anchorType) && (anchorType !== 'pt_peoplecode')) 
 bCallCancelFn = true; }

 } else if (calledFrom === 3) { 
 var cancelFn = function WorkCenterHome(arg1,arg2) {
 top.location.href=arg1; }; } else if (calledFrom === 4) { 
 
 bCallCancelFn = false; } else { 
 return false; }
 ...
 cancelFn(arg1,arg2); }
 return false; }; var processOk = function () {
 var pgltAreaFrame = document.getElementById("ptalPgltAreaFrame"); if (typeof pgltAreaFrame !== "undefined" && pgltAreaFrame != null)
 pgltAreaFrame.contentWindow.hideProcessingIcons(); if (typeof okFn !== "undefined" && okFn != null) {
 okFn(arg1,arg2); return true; }
 return false; }; if(bCallCancelFn)
 processCancel(); if (ptIframe.isWorkCenterDirty()) { 
 var warnMsg = "You have unsaved data on this page. Click OK to go back and save, or Cancel to continue.",
 btnType = "OK_CANCEL";  if (typeof isNewSaveWarn !== "undefined" && isNewSaveWarn) {
 warnMsg = "Do you want to save your changes? Click Yes to go back and save, or No to discard your changes."; btnType = "YES_NO"; }
 ...
 var keyCode = ptUtil.getKeyCode(evt); var actualKeyCode = keyCode | 0x40; var isCtrlKey = ptUtil.isCtrlKey(evt); var isAltKey = ptUtil.isAltKey(evt); if ((isCtrlKey && actualKeyCode == 89) || (!isCtrlKey && isAltKey && actualKeyCode == 90)) { 
 ptIframe.handleNavKeyEvent(actualKeyCode); return false; } else if (isAltKey && (actualKeyCode == 89) && (ptIframe.ptalPage)) {
 
 ptIframe.ptalPage.onTogglePageletArea(); return false; } else {
 ...
 if ((isCtrlKey && keyCode == 89) || (!isCtrlKey && isAltKey && keyCode == 90)) {
 ptIframe.handleNavKeyEvent(keyCode); return false; } else if (isAltKey && (keyCode == 89) && (ptIframe.ptalPage)) {
 
 ptIframe.ptalPage.onTogglePageletArea(); return false; } else if(isAltKey && (keyCode == 112) && (typeof(ptalPage) != 'undefined' && ptalPage != null)) {
 ...
 ptalPage.onShowPageletArea(); } else if(isAltKey && (keyCode == 113) && ptrc.IsPageRCEnabled() && (typeof(RCPage) != 'undefined' && RCPage != null)) {
 ...
 var iframe = ptIframe.resizeIframes[0]; ptIframe.resizeHeight(iframe.name,newHeightOffset); },
 ...
 if (hide) {
 ptIframe.rcIframe.parentNode.style.display = "none"; ptIframe.resizeHeight("ptifrmtgtframe",0); ptIframe.resizeOtherIframes(0); } else {
 ...
 var RCPanelSize = ((typeof RCPage != "undefined") && RCPage && RCPage.objPageletAreaContainer) ? RCPage.objPageletAreaContainer.clientWidth : 0; if (ptIframe.ptalPage) {
 
 ptalPage.WCPanel.resizeBottomRC(tgtHeight, rcHeight); } else if (RCPanelSize > 0){
 RCPage.RCPanel.resizeBottomRC(tgtHeight, rcHeight); } else {

 
 ptIframe.resizeHeight("ptifrmtgtframe",rcHeight); ptIframe.resizeOtherIframes(rcHeight);  if (ptIframe.isRCMin && (!browserInfoObj2.isIE || browserInfoObj2.version > 7)) {
 ptIframe.resizeWidth("RelatedContent",ptIframe.rcIframe.parentNode.offsetLeft); } else {
 ptIframe.resizeWidth("RelatedContent",0); }

 
 ptIframe.rcSep.style.width = ptUtil.getCSSValue(ptIframe.rcIframe,"width");  if (browserInfoObj2.isiPad && browserInfoObj2.isSafari) {
 ...
 ptIframe.rcIframe.style.height = rcHeight - rcSepHeight + "px";  var outifrm = ptUtil.id("ptifrmtgtframe"); var rc = window.frames['RelatedContent']; var RCArea = rc.document.getElementById('RCArea'); if(!RCArea) return; var height = ptIframe.rcIframe.offsetHeight; if (height > ptIframe.rcIframe.offsetTop)
 ...
 ptIframe.rcIframe.parentNode.style.display = "none";  ptIframe.resizeHeight("ptifrmtgtframe",0);  ptIframe.resizeOtherIframes(0); }
 },
